...
import BPMNModdle, { Definitions, FlowNode, Process, StartEvent } from 'bpmn-moddle';

import { SimulationGeneratorService } from './simulation-generator.service';

describe('SimulationGeneratorService', () => {
  let service: SimulationGeneratorService;

  beforeEach(() => {
    TestBed.configureTestingModule({});
    service = TestBed.inject(SimulationGeneratorService);
  });
  ...
  it('should generate simulations', () => {
    const xml = `
    ...
    const promiseFromXML = (moddle as any).fromXML(xml) as any as Promise<{ rootElement: Definitions }>;
    promiseFromXML.then((val => {
      ...
      const elementsArray: Array<FlowNode[]> = service.generate(startEvent);
